...
pub fn exec(runtime: &str, id: &str, cmd: Box<dyn Command>) -> Result<(), libsee::Error> {
    ...
    if let Some(pid) = res {
        ...
        libsee::waitpid(pid, &mut status, 0)?;
        ...
        debug!("stderr: {}", String::from_utf8(line.to_vec()).unwrap_or(format!("{:?}", buf)));
        Ok(())
    } else {
        drop(iomain);
        override_streams(ioworker.streams()).expect("Unable to override streams");
        let argv = cmd.to_argv(runtime, id);
        libsee::execv(&argv)?;
